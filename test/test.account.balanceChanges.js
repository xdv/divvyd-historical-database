...
describe('account balance changes API endpoint', function() {
  ...
  it('should include escrow details', function(done) {
    ...
    function(err, res, body) {
      ...
      assert.strictEqual(body.balance_changes[1].change_type, 'escrow_create')
      assert.strictEqual(body.balance_changes[1].escrow_balance_change, '100')
      assert.strictEqual(body.balance_changes[1].escrow_counterparty,
                         'rGhDCgik9CwiNpcNnYHkEHcMgw2dkLgtNB')
      done()
    })
  })
  ...
  it('should handle pagination correctly', function(done) {
    ...
    utils.checkPagination(url, undefined, function(ref, i, body) {
      ...
      assert.equal(body.balance_changes[0].change,
                   ref.balance_changes[i].change)
      assert.equal(body.balance_changes[0].currency,
                   ref.balance_changes[i].currency)
      assert.equal(body.balance_changes[0].tx_hash,
                   ref.balance_changes[i].tx_hash)
    }, done)
  })
  ...
  it('should handle dates correctly', function(done) {
    ...
    function(err, res, body) {
      ...
      body.balance_changes.forEach(function(bch) {
        ...
        assert.strictEqual(d.isBetween(s, e), true)
      })
      done()
    })
  })
  ...
  it('include a link header when marker is present', function(done) {
    var url = 'http://localhost:' + port +
        '/v2/accounts/rpjZUBy92h6worVCYERZcVCzgzgmHb17Dx' +
        '/balance_changes?limit=1'
    var linkHeader = '<' + url +
      '&marker=rpjZUBy92h6worVCYERZcVCzgzgmHb17Dx' +
      '|20150114182720|000011119603|00004|00001>; rel="next"'
