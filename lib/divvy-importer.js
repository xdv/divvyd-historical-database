...
var Importer = function() {
  ...
  */
  var BackFiller = function (stopIndex, startIndex, callback) {
    ...
    divvyAPI.connect().then(function() {
      getLedger(startIndex + 1);
    });

   /**
    * getLedger
    * get a specific ledger from divvyd
   ...
   /**
    * handleLedger
    * process the ledger returned from divvyd
    */
  ...
  */
  var LiveStream = function () {
    var latest; //latest ledger from divvyd
    var first;  //first ledger from divvyd
    self._active = true;


    log.info("import: starting live stream");
    divvyAPI.connect();
    divvyAPI.on('ledger', function(resp) {
  ...
  self.getLedger = function (options, callback) {
    ...
    function requestLedger(options, callback) {
      ...
      function processLedger(ledger) {
        ...
        try {
          hash = divvyAPI.computeLedgerHash(ledger);
