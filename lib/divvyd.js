...
var url = 'http://' +
    config.get('divvy:server').split('//')[1].split(':')[0] +
    ':51234'
...
module.exports.getBalances = function(options) {
  ...
  return getXdvBalance(options)
  .then(function(resp) {

    if (options.currency && options.currency === 'XDV') {
      return {
...
function adjustQualityForXDV(options) {
  const numeratorShift = (options.pays === 'XDV' ? -6 : 0)
  const denominatorShift = (options.gets === 'XDV' ? -6 : 0)
...
function formatOrder(account, d) {
  ...
  return {
    ...
    properties: {
      ...
      flags: d.flags,
      sequence: d.seq,
      makerExchangeRate: adjustQualityForXDV({
        quality: d.quality,
        gets: gets.currency,
        pays: pays.currency
      })
...
function getXdvBalance(options) {
  var params = {
    method: 'account_info',
    params: [{
      account: options.account,
