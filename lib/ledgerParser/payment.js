...
var Payments = function (tx) {
  ...
  balanceChanges[tx.Account].forEach(function(change) {
    if (change.currency === 'XDV') {
      var fee      = new BigNumber (tx.Fee).dividedBy(XDV_ADJUST).negated();
  ...
  if (typeof tx.Amount === 'object') {
    ...
    payment.currency = 'XDV';
    payment.amount   = new BigNumber(tx.Amount).dividedBy(XDV_ADJUST).toString();
  }
  ...
  if (typeof amount === 'object') {
    ...
    payment.delivered_amount = new BigNumber(amount).dividedBy(XDV_ADJUST).toString();
  }
  ...
  if (typeof tx.SendMax === 'object') {
    ...
    payment.max_amount      = new BigNumber(tx.SendMax).dividedBy(XDV_ADJUST).toString();
    payment.source_currency = 'XDV';
  }
  ...
  payment.fee = new BigNumber (tx.Fee).dividedBy(XDV_ADJUST).toString();
