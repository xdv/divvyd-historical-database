...
var Offers = function (tx) {
  ...
  for (var i=0; i<tx.metaData.AffectedNodes.length; i++) {
    ...
    if (typeof fields.TakerPays === 'object') {
      ...
      offer.taker_pays = {
        currency: 'XDV',
        value: new BigNumber(fields.TakerPays).dividedBy(XDV_ADJUST).toString()
      };
    }
    ...
    if (typeof fields.TakerGets === 'object') {
      ...
      offer.taker_gets = {
        currency: 'XDV',
        value: new BigNumber(fields.TakerGets).dividedBy(XDV_ADJUST).toString()
      };
    }
    ...
    if (node.PreviousFields) {
      ...
      } else if (offer.taker_pays.currency === 'XDV') {
      ...
      } else if (offer.taker_gets.currency === 'XDV') {
