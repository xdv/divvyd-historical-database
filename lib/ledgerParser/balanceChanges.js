...
function BalanceChanges(tx) {
  ...
  function findType(data) {
    ...
    // offer creates are all exchanges
      ...
      // payment currency = XDV and destination account
      } else if (data.account === tx.Destination &&
                 !tx.Amount.currency &&
                 data.currency === 'XDV') {
        return 'payment_destination'
      ...
      // source currency = XDV and source account
      } else if (data.account === tx.Account &&
                tx.SendMax &&
                data.currency === 'XDV') {
        return 'payment_source'
  ...
  function parseAccountRoot(node) {
    ...
    if (tx.Account === account) {
      ...
      list.push({
        account: account,
        currency: 'XDV',
        change: fee.dividedBy(XDV_ADJUST).toString(),
        final_balance: balance.minus(amount).dividedBy(XDV_ADJUST).toString(),
    ...
    if (!amount.isZero()) {
      data = {
        account: account,
        currency: 'XDV',
        change: amount.dividedBy(XDV_ADJUST).toString(),
        final_balance: balance.dividedBy(XDV_ADJUST).toString(),
        time: tx.executed_time,
        ledger_index: tx.ledger_index,
        tx_index: tx.tx_index,
